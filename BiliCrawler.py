...
class Example(QMainWindow):
    ...
    def menu(self):
        ...
        self.UserAct = QAction('获取用户(&U)', self)
    ...
    def StartSpider_user(self):
        ...
        for videoInfo in self.UserVideoList:
            message = videoInfo['bvid'] + ' ' + videoInfo['length'] + '\tplay:' + str(videoInfo['play']) + '\tcomment:' + str(videoInfo['comment'])
            ...
            if not self.centralUI_user.getTotal:
                ...
                fflayout_inner_left.addRow(QLabel(videoInfo['bvid']))
            pass
...
class CentralWidget_user(QWidget):
    ...
    def Init_UI(self):
        ...
        self.checkbox_onlyID.stateChanged.connect(self.getOnlyID)
        self.vlayout.addWidget(self.bt_start)
        self.vlayout.addWidget(self.checkbox_onlyID)
